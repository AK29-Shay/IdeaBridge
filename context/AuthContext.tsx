...
function getFriendlyAuthErrorMessage(message: string) {
  ...
  if (normalized.includes("invalid login credentials")) {
    return "Invalid email or password. If this is a new account, verify your email first or register before signing in.";
  }

  if (normalized.includes("email not confirmed")) {
    return "Email not verified yet. Complete email verification and then sign in.";
  }
  ...
  const value = React.useMemo<AuthContextValue>(
    () => ({
      ...
      async login({ email, password }) {
        ...
        if (error || !data.user) {
          throw new Error(getFriendlyAuthErrorMessage(error?.message || "Login failed."));
        }
